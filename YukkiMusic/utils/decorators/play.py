#
# Copyright (C) 2021 by TeamYukki@Github, < https://github.com/TeamYukki >.
#
# This file is part of < https://github.com/TeamYukki/YukkiMusic > project,
# and is released under the MIT License.
# Please see < https://github.com/TeamYukki/YukkiMusic/blob/master/LICENSE >
#
# All rights reserved.
#
    ...
    async def wrapper(client, message):
        ...
        if await is_maintenance() is False:
            if message.from_user.id not in SUDOERS:
                return await message.reply_text(
                    text=f"{app.mention} is under maintenance.",
                    disable_web_page_preview=True,
                )
        if PRIVATE_BOT_MODE == str(True):
            if not await is_served_private_chat(message.chat.id):
                await message.reply_text(
                    "**Private Music Bot**\n\nOnly For Authorize Chats."
                )
        ...
        try:
            ...
            if not is_call_active:
                return await message.reply_text(
                    f"**No Active VideoChat Found.**\n\nPlease Make Sure You Started The VC."
                )
        except Exception:
            pass
